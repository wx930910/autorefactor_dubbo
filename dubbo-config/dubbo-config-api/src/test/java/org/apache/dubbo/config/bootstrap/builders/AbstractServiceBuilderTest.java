...
class AbstractServiceBuilderTest {
	...
	static private AbstractServiceConfig mockAbstractServiceConfig1() {
	...
	void build() {
		...
		builder.version("version").group("group").deprecated(true).delay(1000).export(false).weight(1)
				.document("document").dynamic(true).token("token").accesslog("accesslog").addProtocol(protocol)
				.protocolIds("protocolIds").tag("tag").executes(100).register(false).warmup(200)
				.serialization("serialization").id("id").prefix("prefix");

		AbstractServiceConfig config = builder.build();
		AbstractServiceConfig config2 = builder.build();
	...
	private static class ServiceBuilder extends AbstractServiceBuilder<AbstractServiceConfig, ServiceBuilder> {

		public AbstractServiceConfig build() {
			AbstractServiceConfig parameterConfig = AbstractServiceBuilderTest.mockAbstractServiceConfig1();
