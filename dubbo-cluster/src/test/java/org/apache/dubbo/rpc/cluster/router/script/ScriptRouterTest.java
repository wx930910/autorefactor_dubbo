...
public class ScriptRouterTest {
	...
	public void testRouteReturnAll() {
		Router router = new ScriptRouterFactory()
				.getRouter(getRouteUrl("function route(op1,op2){return op1} route(invokers)"));
	...
	public void testRoutePickInvokers() {
		String rule = "var result = new java.util.ArrayList(invokers.size());" + "for (i=0;i<invokers.size(); i++){ "
				+ "if (invokers.get(i).isAvailable()) {" + "result.add(invokers.get(i)) ;" + "}" + "} ; "
				+ "return result;";
		...
		List<Invoker<String>> invokers = new ArrayList<Invoker<String>>();
		Invoker<String> invoker1 = MockInvoker.mockInvoker3(false);
		Invoker<String> invoker2 = MockInvoker.mockInvoker3(true);
		Invoker<String> invoker3 = MockInvoker.mockInvoker3(true);
	...
	public void testRouteHostFilter() {
		...
		Invoker<String> invoker1 = MockInvoker
				.mockInvoker1(URL.valueOf("dubbo://10.134.108.1:20880/com.dubbo.HelloService"));
		Invoker<String> invoker2 = MockInvoker
				.mockInvoker1(URL.valueOf("dubbo://10.134.108.2:20880/com.dubbo.HelloService"));
		Invoker<String> invoker3 = MockInvoker
				.mockInvoker1(URL.valueOf("dubbo://10.134.108.3:20880/com.dubbo.HelloService"));
		invokers.add(invoker1);
		invokers.add(invoker2);
		invokers.add(invoker3);
		...
		String script = "function route(invokers, invocation, context){ "
				...
				+ "	var targetHost = new java.util.ArrayList(); " + "	targetHost.add(\"10.134.108.2\"); "
				+ "	for (var i = 0; i < invokers.length; i++) { "
				+ "		if(targetHost.contains(invokers[i].getUrl().getHost())){ "
				+ "			result.add(invokers[i]); " + "		} " + "	} " + "	return result; " + "} "
