...
public class RegistryDirectoryTest {
	...
	static public Registry mockRegistry2(boolean destroyWithError) {
		boolean[] mockFieldVariableDestroyWithError = new boolean[1];
		CountDownLatch mockFieldVariableLatch = null;
		Registry mockInstance = Mockito.spy(Registry.class);
		mockFieldVariableDestroyWithError[0] = destroyWithError;
		try {
			...
			Mockito.doAnswer((stubInvo) -> {
				return null;
			}).when(mockInstance).lookup(Mockito.any());
			Mockito.doNothing().when(mockInstance).unregister(Mockito.any());
			Mockito.doAnswer((stubInvo) -> {
				return null;
			}).when(mockInstance).getUrl();
			Mockito.doAnswer((stubInvo) -> {
				if (mockFieldVariableDestroyWithError[0]) {
					throw new RpcException("test exception ignore.");
				}
				return null;
			}).when(mockInstance).destroy();
			Mockito.doNothing().when(mockInstance).register(Mockito.any());
			Mockito.doNothing().when(mockInstance).subscribe(Mockito.any(), Mockito.any());
			Mockito.doAnswer((stubInvo) -> {
				if (mockFieldVariableLatch != null)
					mockFieldVariableLatch.countDown();
				return null;
			}).when(mockInstance).unsubscribe(Mockito.any(), Mockito.any());
		} catch (Exception exception) {
		}
		return mockInstance;
	}

	static public Registry mockRegistry1(CountDownLatch latch) {
		boolean mockFieldVariableDestroyWithError = false;
		CountDownLatch[] mockFieldVariableLatch = new CountDownLatch[1];
		Registry mockInstance = Mockito.spy(Registry.class);
		mockFieldVariableLatch[0] = latch;
		try {
			Mockito.doAnswer((stubInvo) -> {
				return true;
			}).when(mockInstance).isAvailable();
			...
			Mockito.doAnswer((stubInvo) -> {
				if (mockFieldVariableDestroyWithError) {
					throw new RpcException("test exception ignore.");
				}
				return null;
			}).when(mockInstance).destroy();
			...
			Mockito.doNothing().when(mockInstance).subscribe(Mockito.any(), Mockito.any());
			Mockito.doAnswer((stubInvo) -> {
				if (mockFieldVariableLatch[0] != null)
					mockFieldVariableLatch[0].countDown();
				return null;
			}).when(mockInstance).unsubscribe(Mockito.any(), Mockito.any());
		} catch (Exception exception) {
		}
	...
	URL SERVICEURL2 = URL
			.valueOf("dubbo://127.0.0.1:9092/" + service + "?lazy=true&side=consumer&application=mockName");
	URL SERVICEURL3 = URL
			.valueOf("dubbo://127.0.0.1:9093/" + service + "?lazy=true&side=consumer&application=mockName");
	URL SERVICEURL_DUBBO_NOPATH = URL
			.valueOf("dubbo://127.0.0.1:9092" + "?lazy=true&side=consumer&application=mockName");
	...
	public void test_Constructor_CheckStatus() throws Exception {
		...
		Assertions.assertEquals(url.setProtocol(CONSUMER_PROTOCOL).clearParameters().addParameter("foo", "bar"),
				reg.getConsumerUrl());
	}
	...
	private void test_Notified1invokers(RegistryDirectory registryDirectory) {
		...
		List<URL> serviceUrls = new ArrayList<URL>();
		serviceUrls.add(
				SERVICEURL.addParameter("methods", "getXXX1").addParameter(APPLICATION_KEY, "mockApplicationName"));// .addParameter("refer.autodestroy",
																													// "true")
		registryDirectory.notify(serviceUrls);
		Assertions.assertTrue(registryDirectory.isAvailable());
	...
	public void testParametersMerge() {
		...
		RegistryDirectory<RegistryDirectoryTest> registryDirectory2 = new RegistryDirectory(RegistryDirectoryTest.class,
				regurl);
		...
		// test geturl
		{
			...
			Assertions.assertEquals("true",
					((InvokerWrapper<?>) registryDirectory2.getInvokers().get(0)).getUrl().getParameter("mock"));
		}
	}

	/**
	 * When destroying, RegistryDirectory should: 1. be disconnected from Registry
	...
	public void testDestroy_WithDestroyRegistry() {
		...
		registryDirectory.setRegistry(RegistryDirectoryTest.mockRegistry1(latch));
		registryDirectory
				.subscribe(URL.valueOf("consumer://" + NetUtils.getLocalHost() + "/DemoService?category=providers"));
		registryDirectory.destroy();
	...
	public void testDestroy_WithDestroyRegistry_WithError() {
		RegistryDirectory registryDirectory = getRegistryDirectory();
		registryDirectory.setRegistry(RegistryDirectoryTest.mockRegistry2(true));
		registryDirectory.destroy();
	}
...
//        );
	...
	/**
	 * When the first arg of a method is String or Enum, Registry server can do
	...
	/**
	 * 1. notify twice, the second time notified router rules should completely
	 * replace the former one. 2. notify with no router url, do nothing to current
	 * routers 3. notify with only one router url, with router=clean, clear all
	 * current routers
	 */
	@Test
	public void testNotifyRouterUrls() {
		...
		serviceUrls.add(routerurl.addParameter(CATEGORY_KEY, ROUTERS_CATEGORY).addParameter(TYPE_KEY, "javascript")
				.addParameter(ROUTER_KEY, "notsupported").addParameter(RULE_KEY, "function test1(){}"));
		serviceUrls.add(routerurl2.addParameter(CATEGORY_KEY, ROUTERS_CATEGORY).addParameter(TYPE_KEY, "javascript")
				.addParameter(ROUTER_KEY, ScriptRouterFactory.NAME).addParameter(RULE_KEY, "function test1(){}"));

		// FIXME
		/*
		 * registryDirectory.notify(serviceUrls); RouterChain routerChain =
		 * registryDirectory.getRouterChain(); //default invocation selector
		 * Assertions.assertEquals(1 + 1, routers.size());
		 * Assertions.assertTrue(ScriptRouter.class == routers.get(1).getClass() ||
		 * ScriptRouter.class == routers.get(0).getClass());
		 * 
		 * registryDirectory.notify(new ArrayList<URL>()); routers =
		 * registryDirectory.getRouters(); Assertions.assertEquals(1 + 1,
		 * routers.size()); Assertions.assertTrue(ScriptRouter.class ==
		 * routers.get(1).getClass() || ScriptRouter.class ==
		 * routers.get(0).getClass());
		 * 
		 * serviceUrls.clear();
		 * serviceUrls.add(routerurl.addParameter(Constants.ROUTER_KEY,
		 * Constants.ROUTER_TYPE_CLEAR)); registryDirectory.notify(serviceUrls); routers
		 * = registryDirectory.getRouters(); Assertions.assertEquals(0 + 1,
		 * routers.size());
		 */
	}

	/**
	 * Test whether the override rule have a high priority Scene: first push
	 * override , then push invoker
	 */
	@Test
	public void testNotifyoverrideUrls_beforeInvoker() {
		...
		// The registry is initially pushed to override only, and the dirctory state
		// should be false because there is no invoker.
		Assertions.assertFalse(registryDirectory.isAvailable());
		...
		Assertions.assertEquals("1", invokers.get(0).getUrl().getParameter("timeout"),
				"override rute must be first priority");
		Assertions.assertEquals("5", invokers.get(0).getUrl().getParameter("connections"),
				"override rute must be first priority");
	}
	...
	/**
	 * Test whether the override rule have a high priority Scene: push override
	 * rules with invoker
	 */
	@Test
	...
	/**
	 * Test whether the override rule have a high priority Scene: the rules of the
	 * push are the same as the parameters of the provider Expectation: no need to
	 * be re-referenced
	 */
	@Test
	...
	public void testNofityOverrideUrls_Provider() {
		...
		List<URL> durls = new ArrayList<URL>();
		durls.add(
				SERVICEURL.setHost("10.20.30.140").addParameter("timeout", "1").addParameter(SIDE_KEY, CONSUMER_SIDE));// One
																														// is
																														// the
																														// same,
																														// one
																														...
																														// different
		durls.add(
				SERVICEURL2.setHost("10.20.30.141").addParameter("timeout", "2").addParameter(SIDE_KEY, CONSUMER_SIDE));
		registryDirectory.notify(durls);
	...
	public void testNofityOverrideUrls_CleanOnly() {
		...
		registryDirectory.notify(durls);
		Assertions
				.assertNull(((InvokerWrapper<?>) registryDirectory.getInvokers().get(0)).getUrl().getParameter("mock"));
		...
		Assertions.assertEquals("fail",
				((InvokerWrapper<?>) registryDirectory.getInvokers().get(0)).getUrl().getParameter("mock"));
	...
	/**
	 * Test the simultaneous push to clear the override and the override for a
	 * certain provider See if override can take effect
	 */
	@Test
	...
	/**
	 * Test override disables a specified service provider through enable=false It
	 * is expected that a specified service provider can be disable.
	 */
	@Test
	public void testNofityOverrideUrls_disabled_specifiedProvider() {
		...
		durls = new ArrayList<URL>();
		durls.add(URL
				.valueOf("empty://0.0.0.0?" + DISABLED_KEY + "=true&" + CATEGORY_KEY + "=" + CONFIGURATORS_CATEGORY));
		registryDirectory.notify(durls);
	...
	public void testNotifyRouterUrls_Clean() {
		if (isScriptUnsupported)
			return;
		RegistryDirectory registryDirectory = getRegistryDirectory();
		URL routerurl = URL.valueOf(ROUTE_PROTOCOL + "://127.0.0.1:9096/").addParameter(ROUTER_KEY, "javascript")
				.addParameter(RULE_KEY, "function test1(){}").addParameter(ROUTER_KEY, "script"); // FIX
		// BAD
		...
		// FIXME
		/*
		 * List routers = registryDirectory.getRouters(); Assertions.assertEquals(1 + 1,
		 * routers.size());
		 * 
		 * serviceUrls.clear();
		 * serviceUrls.add(routerurl.addParameter(Constants.ROUTER_KEY,
	...
	public void test_Notified_withGroupFilter() {
		URL directoryUrl = noMeaningUrl.addParameterAndEncoded(REFER_KEY,
				"interface" + service + "&group=group1,group2");
		RegistryDirectory directory = this.getRegistryDirectory(directoryUrl);
		URL provider1 = URL.valueOf("dubbo://10.134.108.1:20880/" + service
				+ "?methods=getXXX&group=group1&mock=false&application=mockApplication");
		URL provider2 = URL.valueOf("dubbo://10.134.108.1:20880/" + service
				+ "?methods=getXXX&group=group2&mock=false&application=mockApplication");
