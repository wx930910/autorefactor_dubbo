	...
	static public ServiceDiscovery mockServiceDiscovery1() {
		...
		try {
			Mockito.doAnswer((stubInvo) -> {
				...
				return null;
			}).when(mockInstance).register(Mockito.any(ServiceInstance.class));
			Mockito.doAnswer((stubInvo) -> {
				ServiceInstance serviceInstance = stubInvo.getArgument(0);
				mockInstance.unregister(serviceInstance);
				mockInstance.register(serviceInstance);
				return null;
			}).when(mockInstance).update(Mockito.any(ServiceInstance.class));
			Mockito.doAnswer((stubInvo) -> {
			...
			Mockito.doAnswer((stubInvo) -> {
				...
				return null;
			}).when(mockInstance).initialize(Mockito.any(URL.class));
			Mockito.doAnswer((stubInvo) -> {
				ServiceInstance serviceInstance = stubInvo.getArgument(0);
				String serviceName = serviceInstance.getServiceName();
				List<ServiceInstance> serviceInstances = mockFieldVariableRepository.computeIfAbsent(serviceName,
				...
				return null;
			}).when(mockInstance).unregister(Mockito.any(ServiceInstance.class));
